...
self.addEventListener('message', ({ data }: MessageEvent<WorkerRequest>) => {
  ...
  if (!world_ready) throw new Error('mesh worker received work before its world recipe')
  void world_ready
    .then(async (world) => {
      ...
      await load_generated_city_artifacts_for(world.structures.cities, area)
      let structures = structure_cache.get(horizontal_key)
      if (!structures) {
        if (structure_cache.size > 128) structure_cache.clear()
        structures = structure_placements(world, area)
