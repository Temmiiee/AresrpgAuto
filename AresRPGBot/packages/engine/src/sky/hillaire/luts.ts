...
import { Storage3DTexture, StorageTexture, type Node } from 'three/webgpu'
import {
  Fn,
  Loop,
  clamp,
...
import type { AtmoUniforms } from './physics.ts'
import type { SkyTier } from './atmosphere_params.ts'
 ...
 */
export type DynamicSkyUniforms = Readonly<{
  sun_dir: Node<'vec3'>
  cam_height_km: Node<'float'>
  cam_fwd: Node<'vec3'>
  cam_right: Node<'vec3'>
  cam_up: Node<'vec3'>
  tan_half_x: Node<'float'>
  tan_half_y: Node<'float'>
  aerial_range_km: Node<'float'>
  aerial_depth_power: Node<'float'>
  units_to_km: Node<'float'>
}>

export function create_luts({ U, dyn, tier }: { U: AtmoUniforms; dyn: DynamicSkyUniforms; tier: SkyTier }) {
  ...
  const sample_transmittance = (r: Node<'float'>, mu: Node<'float'>): Node<'vec3'> =>
    texture(transmittance, transmittance_params_to_uv(U, r, mu), 0).xyz
  /** Multiple-scattering Psi_ms at radius `r` for sun zenith-cosine `sun_cos`. @param {*} r @param {*} sun_cos @returns {*} vec3 */
  const sample_multiscatter = (r: Node<'float'>, sun_cos: Node<'float'>): Node<'vec3'> => {
  ...
  const sun_transmittance = (r: Node<'float'>, sun_cos: Node<'float'>): Node<'vec3'> =>
   ...
   */
  const sample_skyview = (dir: Node<'vec3'>): Node<'vec3'> => {
    const cam_r = dyn.cam_height_km.add(ground)
    ...
    // Azimuth is undefined at either zenith. A guarded zero vector selects the
    // neutral LUT meridian instead of allowing normalize(0) to poison the sky.
    const vh_safe = vh.div(length(vh).max(float(1e-6)))
    const sh_safe = sh.div(length(sh).max(float(1e-6)))
    const lvc = clamp(vh_safe.dot(sh_safe), -1, 1)
   ...
   */
  const sample_aerial = (screen_uv: Node<'vec2'>, dist_km: Node<'float'>) => {
