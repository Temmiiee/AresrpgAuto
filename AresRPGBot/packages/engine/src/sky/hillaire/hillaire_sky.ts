...
import {
  ...
  type SkyTier,
} from './atmosphere_params.ts'
import { create_luts } from './luts.ts'
 ...
 *  "broaden the glow so we don't see a yellow circle" — no hard yellow-circle boundary). */
...
//   • MIE: a pow(cosθ) tail with no window — bright out to ≈10° at noon (2.5× the old ~4°
...
/** the night moon body: cool white-grey and soft — "soft blue grey light", not a second sun.
 *  INNER/limb soften the disc; a faint halo.
...
export type HillaireSkyOptions = Readonly<{
  tier?: 'low' | 'medium' | 'high' | SkyTier
  seed?: string | number
...
export function create_hillaire_sky(opts: HillaireSkyOptions = {}) {
  ...
  let k_transmittance: ComputeNode | null = null
  let k_multiscatter: ComputeNode | null = null
  let k_skyview: ComputeNode | null = null
  let k_aerial: ComputeNode | null = null
  ...
  // ── consumer 1: background = sky-view LUT + sun disc/corona + the night moon ───────────────────────
  ...
  // circle), bright enough to form a large soft sun directly.
  ...
  const set_atmosphere_params = (partial: Partial<AtmosphereParams>): void => {
  ...
  const update_dynamic = (camera: PerspectiveCamera): void => {
