...
import { clamp, mix, positionWorldDirection, pow, smoothstep, texture, uniform, vec3 } from 'three/tsl'

// Extraction bridge: keep the exact legacy photographic source until binary assets move into seed/models.
// Runtime logic no longer imports deprecated code; only this immutable asset URL crosses the seam.
import { DAY_FRAC, sun_dir_from_tod } from './celestial_motion.ts'
...
export { DAY_FRAC, sun_dir_from_tod } from './celestial_motion.ts'
export { SKY_NIGHT, disc_space_uv, disc_space_uv_js, type Rgb, type SkyPalette } from './shared.ts'
...
export const SUN_TINT_DAY: Rgb = [1.0, 0.96, 0.9]
/** deep-orange sun tint at dusk (reddened by the long horizon path). @type {Rgb} */
export const SUN_TINT_DUSK: Rgb = [1.0, 0.5, 0.24]
 ...
 *  the sun's forward glow reads powerful and forms a cinematic halo directly. The
...
export { MOON_DISC_COS, MOON_DISC_RGB, MOON_ANGULAR_RADIUS, MOON_DISC_INNER_COS, MOON_LIMB_EDGE, moon_texture }
...
const saturate = (x: number): number => clampf(x, 0, 1)
/**
 ...
 * @returns {SkyPalette}
 */
export function palette_for_sun(sun_y: number): SkyPalette {
  const to_dusk = smooth(-0.18, -0.02, sun_y)
  const to_day = smooth(-0.02, 0.22, sun_y)
  /** @param {keyof SkyPalette} k @returns {Rgb} */
  const stop = (k: keyof SkyPalette): Rgb => mix3(mix3(SKY_NIGHT[k], SKY_DUSK[k], to_dusk), SKY_DAY[k], to_day)
 ...
 */
export function sun_tint_for(sun_y: number): Rgb {
 ...
 * clamp (the tone mapper consumes the halo peak downstream).
 * @param {Rgb} view unit view direction (world)
 * @param {Rgb} sun unit sun direction (world)
 * @returns {Rgb} linear radiance
 */
export function sample_sky_rgb(view: Rgb, sun: Rgb): Rgb {
  const [, cos_up] = view
...
export function create_sky_node({ initial_tod = 0.3, seed = 'aresrpg' } = {}) {
  ...
  const stop_node = (k: keyof SkyPalette, sun_y: Node<'float'>): Node<'vec3'> =>
    mix(
   ...
   *  elimination — it survived every post-process cap identically, so it was never a post artifact): the ~15°-wide
  ...
  const sample_sky_dome = (dir: Node<'vec3'>): Node<'vec3'> => {
