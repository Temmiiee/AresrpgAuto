...
import {
  DOFUS_GEAR_POWER,
  DOFUS_POWER_SCALE,
  DOFUS_WEAPON_DAMAGE_SCALE,
  DOFUS_WEAPON_POWER,
} from './dofus_item_power_corpus.gen.ts'
...
type WeaponDonor = readonly [
  ...
  merged_average_damage_scaled: number,
  merged_maximum_damage_scaled: number,
]

const MIN_COHORT = 20
const LEVEL_RADIUS = 10
const gear_by_category = DOFUS_GEAR_POWER as Readonly<Record<string, readonly GearDonor[]>>
const weapon_by_family = DOFUS_WEAPON_POWER as Readonly<Record<string, readonly WeaponDonor[]>>
const weapon_donor_families = Object.freeze({
  bow: 'bow',
  spear: 'staff',
  daggers: 'daggers',
  sword: 'sword',
...
const weapon_family = (category: string): string | null =>
  weapon_donor_families[category as keyof typeof weapon_donor_families] ?? null
const weapon_rows = (category: string): readonly WeaponDonor[] => {
  const family = weapon_family(category)
  return family ? (weapon_by_family[family] ?? []) : []
}

const gear_rows = (category: string): readonly GearDonor[] => {
  const gear = gear_by_category[category]
  if (gear) return gear
  const weapon = weapon_rows(category)
  return weapon.length ? weapon.map(([level, power]) => [level, power]) : all_gear
}
...
const quantile = (values: readonly number[], percentile: number, scale = DOFUS_POWER_SCALE): number => {
...
export const item_budget_envelope = (level: number, category = ''): ItemBudgetEnvelope => {
  const gear = gear_by_category[category]
  const family = weapon_family(category)
  const comparison = gear ? category : (family ?? 'all gear')
...
export const item_budget_standing = (level: number, category: string, maximum: number): ItemBudgetStanding => {
  ...
  const direct = gear_by_category[category] ?? weapon_rows(category)
  return Object.freeze({
...
export const dofus_weapon_damage_envelope = (
  level: number,
  category: string,
  ap: number
): WeaponDamageEnvelope | null => {
  const donor_family = weapon_family(category)
  const rows = weapon_rows(category)
  if (!donor_family || !rows.length || !Number.isFinite(ap) || ap <= 0) return null
  const selected = cohort(rows, Math.max(1, level))
  const average = selected.map(([, , value]) => value)
  const maximum = selected.map(([, , , value]) => value)
  const damage_scale = DOFUS_WEAPON_DAMAGE_SCALE * ap
  return Object.freeze({
    donor_family,
    average_p10: quantile(average, 0.1, damage_scale),
    average_median: quantile(average, 0.5, damage_scale),
    average_p90: quantile(average, 0.9, damage_scale),
    average_max: Math.max(...average) / damage_scale,
    maximum_p10: quantile(maximum, 0.1, damage_scale),
    maximum_median: quantile(maximum, 0.5, damage_scale),
    maximum_p90: quantile(maximum, 0.9, damage_scale),
    maximum_max: Math.max(...maximum) / damage_scale,
    sample_count: selected.length,
    ...bounds(selected),
  })
}
