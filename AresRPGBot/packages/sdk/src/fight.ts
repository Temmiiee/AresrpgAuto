...
export type RolledDrop = Readonly<{ item_type: string; qty: number }>
export type FighterDrops = Readonly<{ fighter: bigint; drops: readonly RolledDrop[] }>
export type FightReceipt = {
  ...
  closable?: boolean
  closed?: boolean
  /** Only present on a settle() receipt that actually rolled loot (fight.move's DropsRolled
   *  event — emitted once, on the single settle call in the whole fight where drops_rolled
   *  flips false->true, one event per winning seat). Never present otherwise. */
  drops_rolled?: readonly FighterDrops[]
}
...
const rolled_drops = (receipt: Receipt): readonly FighterDrops[] =>
  Object.freeze(
    receipt_events(receipt, '::fight::DropsRolled').map((event) => {
      if (typeof event.fighter !== 'string' || !Array.isArray(event.drops))
        throw new Error('The settle receipt carried a malformed DropsRolled event.')
      const drops = event.drops.map((raw) => {
        const row = raw as Record<string, unknown>
        return Object.freeze({ item_type: String(row.item_type), qty: Number(row.qty) })
      })
      return Object.freeze({ fighter: BigInt(event.fighter), drops: Object.freeze(drops) })
    })
  )
...
export const last_settler_refusal = (error: unknown): boolean => {
  ...
  return before_submission && /abort code:\s*(1729|131072)\b/i.test(message)
}
  ...
  return {
    ...
    settle: async ({
      ...
      return Object.freeze({
        ...
        drops_rolled: rolled_drops(receipt),
      })
    },
